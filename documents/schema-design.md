...
> **v0.7 → v0.8:** referrals 테이블 reward_stage 도입 (referrer_rewarded/referred_rewarded 제거), daily_checkins 일일 룰렛 모델로 변경 (streak_count/roulette_earned 제거, reward_amount 추가), user_monthly_checkin_caps 테이블 신규, point_transactions type 개정
...
| **보상** | user_points, point_transactions, referrals, daily_checkins, user_monthly_checkin_caps, roulette_results | 센트(¢) + 추천 + 일일 출석 룰렛 + 숨겨진 월한도 |
...
| 값 | amount | 설명 |
|---|---|---|
| `daily_checkin` | +1 | 일일 출석 룰렛 당첨 보상 (1¢) |
| `roulette_event` | +N | 이벤트/퀴즈 룰렛 당첨 보상 |
| `referral_welcome` | +1 | 피초대자 가입 웰컴 보상 (1¢) |
| `referral_purchase_referred` | +1 | 피초대자 구매 달성 보상 (Stage 1, 2 각 +1¢) |
| `referral_purchase_referrer` | +N | 초대자 구매 달성 보상 (Stage 1: 2¢, Stage 2: 3¢) |
| `signup_bonus` | +N | 가입 보너스 |
| `gifticon_exchange` | -N | 기프티콘 교환에 사용 |
| `ad_removal` | -N | 광고 제거에 사용 |
| `admin_adjustment` | ±N | 운영자 수동 조정 |

### 2-13. referrals (추천 보상 추적) ★ 신규 (v0.8 개정)
...
| reward_stage | SMALLINT | NOT NULL, DEFAULT 0 | 보상 진행 단계 (0=가입만, 1=첫구매 보상완료, 2=두번째구매 보상완료) |
| created_at | TIMESTAMPTZ | NOT NULL, DEFAULT NOW() | |

**CHECK 제약:** `chk_referrals_reward_stage`: `reward_stage BETWEEN 0 AND 2`

**설계 근거:**
- `referred_id` UNIQUE — 한 사용자는 한 번만 추천받을 수 있음
- **단계별 구매 기반 보상**:
  - Stage 0 (가입): 피초대자 1¢ 웰컴 지급
  - Stage 1 (1만원 이상 첫 구매): 피초대자 +1¢, 초대자 2¢ 지급
  - Stage 2 (1만원 이상 두번째 구매): 피초대자 +1¢, 초대자 3¢ 지급 (완료)
- `reward_stage`로 현재 진행 상태를 추적. 구매 확인 이벤트 발생 시 단계 업그레이드 + 보상 지급

### 2-14. daily_checkins (일일 출석 룰렛) ★ 신규 (v0.8 개정)
...
| reward_amount | SMALLINT | NOT NULL, DEFAULT 0 | 당일 룰렛 결과 (0¢ 또는 1¢) |
| created_at | TIMESTAMPTZ | NOT NULL, DEFAULT NOW() | |

**UNIQUE 제약:** `(user_id, checkin_date)` — 하루 1회만

**설계 근거:**
- 출석 + 룰렛 통합: 매일 앱 방문 시 즉시 룰렛 1회 실행
- `reward_amount`로 당일 룰렛 결과 기록 (0¢ 미당첨 또는 1¢ 당첨)
- 월한도는 `user_monthly_checkin_caps` 테이블에서 별도 관리

### 2-14a. user_monthly_checkin_caps (숨겨진 월별 출석 한도) ★ 신규

| 컬럼 | 타입 | 제약조건 | 설명 |
|---|---|---|---|
| id | BIGINT | PK, GENERATED ALWAYS AS IDENTITY | |
| user_id | BIGINT | FK -> users(id), NOT NULL | |
| year_month | TEXT | NOT NULL | 적용 월 (예: '2026-03') |
| monthly_cap | SMALLINT | NOT NULL | 해당 월 센트 한도 (1~4) |
| earned_so_far | SMALLINT | NOT NULL, DEFAULT 0 | 해당 월 현재까지 획득한 센트 |
| created_at | TIMESTAMPTZ | NOT NULL, DEFAULT NOW() | |

**UNIQUE 제약:** `(user_id, year_month)` — 유저당 월 1건

**CHECK 제약:**
- `chk_monthly_cap_range`: `monthly_cap BETWEEN 1 AND 4`
- `chk_earned_range`: `earned_so_far BETWEEN 0 AND monthly_cap`

**설계 근거:**
- **숨겨진 월한도 시스템**: 유저에게 비공개, 서버 내부에서만 관리
- 매월 초(또는 첫 출석 시 lazy 생성) 유저별 월한도 배정
- **배분 확률**: 1¢ = 90%, 2¢ = 6%, 3¢ = 3%, 4¢ = 1%
- **신규 유저**: 가입 첫 달은 1~2¢ 범위에서만 배정
- `earned_so_far >= monthly_cap`이면 해당 월 룰렛 결과 항상 0¢
...
| idx_daily_checkins_user | daily_checkins | (user_id, checkin_date DESC) | 사용자 출석 이력 + 월별 합산 |
| idx_monthly_caps_user | user_monthly_checkin_caps | (user_id, year_month) | 유저별 월한도 조회 |
...
| daily_checkins | chk_daily_checkins_reward | `reward_amount BETWEEN 0 AND 1` | 003 (v0.8 개정) |
| referrals | chk_referrals_reward_stage | `reward_stage BETWEEN 0 AND 2` | 신규 (v0.8) |
| user_monthly_checkin_caps | chk_monthly_cap_range | `monthly_cap BETWEEN 1 AND 4` | 신규 (v0.8) |
| user_monthly_checkin_caps | chk_earned_range | `earned_so_far BETWEEN 0 AND monthly_cap` | 신규 (v0.8) |
...
| **추천 보상** | referrals + point_transactions | 단계별 구매 기반: 가입 1¢ → 첫구매(초대자 2¢, 피초대자 +1¢) → 두번째구매(초대자 3¢, 피초대자 +1¢) |
| **센트(¢) 시스템** | user_points + point_transactions | 앱 내 보상 화폐 → 기프티콘 교환 |
| **일일 출석 룰렛** | daily_checkins + user_monthly_checkin_caps + roulette_results | 매일 룰렛, 숨겨진 월한도(1~4¢) |
